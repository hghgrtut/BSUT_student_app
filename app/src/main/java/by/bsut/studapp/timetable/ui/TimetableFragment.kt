...
class TimetableFragment : Fragment(R.layout.fragment_timetable) {
    ...
    private var isUpperWeek: Boolean = true
    ...
    private suspend fun refreshParas(view: View) {
        ...
        if (prefEditor.getWeekMode() != isUpperWeek) {
            isUpperWeek = !isUpperWeek
            prefEditor.setWeekMode(isUpperWeek)
            getWeekParasList(isUpperWeek)
        } else if (paras == null) { getWeekParasList(isUpperWeek) }
        ...
        recyclerView.apply { adapter = ParaViewAdapter(todayParas) }
    }

    private suspend fun getWeekParasList(isUpperWeek: Boolean) {
        paras = withContext(Dispatchers.IO) {
            if (isUpperWeek) dao.getUpperWeekParas() else dao.getLowerWeekParas()
        }
    }
}